      ...
      if (filter && !filter.hasAttribute('data-vce-state') && (filter.getAttribute('data-vce-collapsible') === 'true')) {
        var resizeObserver = new ResizeObserver(entries => {
          for (let entry of entries) {
            if (entry.target && entry.contentBoxSize && entry.contentBoxSize[0]) {
              ...
              if (children && children.length) {
                ...
                if (!entry.target.hasAttribute('data-vce-state')) {
                  if (childrenTotalWidth > entry.contentBoxSize[0].inlineSize) {
                    entry.target.setAttribute('data-vce-state', 'dropdown')
                  } else {
                    entry.target.setAttribute('data-vce-state', 'items')
                  }
                } else if ((childrenTotalWidth > entry.contentBoxSize[0].inlineSize) && (entry.target.getAttribute('data-vce-state') === 'items')) {
                  entry.target.setAttribute('data-vce-state', 'dropdown')
                } else if ((childrenTotalWidth < entry.contentBoxSize[0].inlineSize) && (entry.target.getAttribute('data-vce-state') === 'dropdown')) {
                  entry.target.setAttribute('data-vce-state', 'items')
                }
              }
            }
