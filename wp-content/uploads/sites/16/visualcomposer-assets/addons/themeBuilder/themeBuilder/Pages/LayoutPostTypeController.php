...
class LayoutPostTypeController extends Container implements Module
{
    ...
    protected function renderLayouts(Request $requestHelper, Frontend $frontendHelper, Options $optionsHelper)
    {
    ...
    protected function printLayout($keepOriginalPostScope)
    {
        ...
        // Replace layout content with post content
        add_filter(
            'the_content',
            function ($content) use ($layoutContent) {
                $newContent = $layoutContent;
                $replaced = false;
                if (get_the_ID() === $this->currentSourceId
                    && strpos($layoutContent, '{{vcwb-layout-content-area}}') !== false
                ) {
                    $newContent = str_replace('{{vcwb-layout-content-area}}', $content, $newContent);
                    $replaced = true;
                }
