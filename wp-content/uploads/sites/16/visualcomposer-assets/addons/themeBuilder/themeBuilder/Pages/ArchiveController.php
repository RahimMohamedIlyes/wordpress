...
class ArchiveController extends Container implements Module
{
    ...
    public static function isArchivePage()
    {
        ...
        $archiveName = self::getArchiveTemplateName();

        if (isset($archiveName) && !empty($archiveName)) {
            $archiveTemplate = $optionsHelper->get('custom-page-templates-' . $archiveName . '-template', false);

            // Check backward compability
     ...
     * @return string
     */
    public static function getArchiveTemplateName()
    {
        ...
        if ($wp_query->is_posts_page) {
            ...
            if (($pageForPostID !== '0' && $currentPage->ID == $pageForPostID)
                || ($pageForPostID === '0'
                    && is_home())
            ) {
                $isPostPage = true;
        ...
        if (function_exists('is_shop') && is_shop()) {
            ...
            $archiveName = 'search';
        } elseif (is_year() || is_month() || is_day() || $isPostPage) {
            $archiveName = 'post';
        }
