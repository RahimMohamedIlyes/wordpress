...
<script>
  window.VCV_FONT_MANAGER_ACTION = "<?php echo vchelper('Url')->adminAjax(
      ['vcv-action' => 'settings:fontManager:save:adminNonce', 'vcv-nonce' => vchelper('Nonce')->admin()]
  ); ?>";
  ...
  $defaults = vcapp('\fontManager\fontManager\FontManagerEnqueueController')->getFontManagerDefaults();
  $currentValues = array_merge($defaults, $optionsHelper->get('fontManager', $defaults));
...
<script>
  (function () {
    window.VCV_GOOGLE_FONTS_CURRENT = <?php echo json_encode(
        array_values(array_unique(array_column($currentValues, 'font_family')))
    ); ?>;
  })()
</script>
<?php
