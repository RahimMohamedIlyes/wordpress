...
class AdvancedCustomFields extends Container implements Module
{
    ...
    protected function acfGetValue($field, $sourceId, $acfGroup)
    {
        ...
        return $this->autoP($field, $result);
    }
     ...
     * @return array
     */
    protected function replaceFieldsPostData($response)
    {
        foreach ($this->replaceableFieldsList as $fieldData) {
            ...
            $fieldReplaceableDta = $this->getFieldReplaceData($fieldData['fieldType'], $dataThatNeedReplace);
    ...
    protected function addMetaRendering($metaValue, $payload)
    {
        ...
        $metaValue = $this->getFieldReplaceData($fieldData['type'], $metaValue);
     ...
     * @return string
     */
    protected function getFieldReplaceData($fieldType, $dataThatNeedReplace)
    {
        switch ($fieldType) {
            case 'image':
