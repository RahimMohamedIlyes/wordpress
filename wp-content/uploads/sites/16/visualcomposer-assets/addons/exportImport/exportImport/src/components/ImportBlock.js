...
export default class ImportForm extends React.Component {
  ...
  getStatusMessages () {
    ...
    const messages = []
    messages.push(<p key='vcvImportStatusFirstMessage'>{startImportProcessText}</p>)
    this.state.statusMessages.forEach((message, index) => {
      messages.push(<p key={`vcvImportStatusMessage${index}`} dangerouslySetInnerHTML={{ __html: message }} />)
    })
    if (this.state.errorMessage) {
      messages.push(<p key='vcvImportErrorMessage'><strong dangerouslySetInnerHTML={{ __html: this.state.errorMessage }} /></p>)
    }
