...
class AdvancedCustomFields extends Container implements Module
{
     ...
     * @return array
     */
    protected function getFields($sourceId, $payload)
    {
        ...
        foreach ($acfData as $acfGroup) {
            $fields = $this->parseFields($sourceId, $post, $acfGroup, $allowedFieldTypes, $fields);
        }
    ...
    protected function addPostData($response, $payload)
    {
        ...
        $fields = $this->getFields($sourceId, $payload);
        if (is_array($fields) && !empty($fields)) {
            foreach ($fields as $field) {
                $response[ $field['value'] ] = $field['actualValue'];

                if (isset($field['additional'])) {
                    $response[$field['value'] . '-additional'] = json_encode($field['additional']);
                }
            }
        }
    ...
    protected function renderFields($response, $payload)
    {
        ...
        return $this->parseFieldsResponse($response, $fields, $atts);
    }

    protected function parseFieldsResponse($response, $fields, $atts)
    {
        foreach ($fields as $field) {
            if ($atts['value'] === $field['value']) {
                if (isset($atts['currentValue'])) {
                    ...
                    if ($field['fieldType'] == 'wysiwyg') {
                        ...
                        $response = $this->parseAdditionalAttributes($field, $response);
                    }
                ...
                $isBothValueEmpty = $this->isNotValueField($atts['currentValue']) && $field['actualValue'] === 'none';
                if (!$frontendHelper->isPageEditable() && $isBothValueEmpty) {
                    $response = '';
                }
            }
        }
    ...
    /**
     * For some ACF fields we should provide additional attributes to response.
     *
     * @param array $field
     ...
     * @return string
     */
    protected function parseAdditionalAttributes($field, $response)
    {
        switch ($field['fieldType']) {
            case 'image':
                if (!empty($field['additional']['image-caption'])) {
                    $replace = '<figcaption>' . $field['additional']['image-caption'] . '</figcaption>';
                } else {
                    $replace = '<figcaption hidden=""></figcaption>';
                }
                $response = preg_replace('/<figcaption(.*?)<\/figcaption>/', $replace, $response);

                if (!empty($field['additional']['image-alt'])) {
                    $replace = ' alt="' . $field['additional']['image-alt'] . '" ';
                } else {
                    $replace = ' alt="" ';
                }
                $response = preg_replace('/ alt="(.*?)"/', $replace, $response);

                if (!empty($field['additional']['image-title'])) {
                    $replace = ' title="' . $field['additional']['image-title'] . '" ';
                } else {
                    $replace = ' title="" ';
                }
                $response = preg_replace('/ title="(.*?)"/', $replace, $response);
    ...
    /**
     * Check if current dynamic field value is empty.
     * For some act field we have placeholders when value im empty
     * that we should not show on a view page.
     *
     * @param string $value
     * @return bool
     */
    protected function isNotValueField($value)
    {
        $postDataHelper = vchelper('PostData');
        return
            strpos($value, "No Value (acf:field_") === 0 ||
            $value === $postDataHelper->getEditorImagePlaceholder()['url'];
    }
     ...
     * @return array
     */
    protected function parseFields($sourceId, $post, $acfGroup, array $allowedFieldTypes, $fields = [])
    {
        ...
        foreach ($acfFields as $acfField) {
            ...
            if ((empty($value) && $acfGroup['forceAddField']) || !empty($value)) {
                $fieldData = [
                    ...
                    'group' => 'acf',
                    'name' => $acfField['name'],
                    'additional' => $this->getAdditionalFieldData($acfField, $post),
                ];
    ...
    /**
     * We need provide some additional field data that ACF value does not have initially.
     *
     ...
     * @return array
     */
    protected function getAdditionalFieldData($acfField, $post)
    {
        $additionalData = [];
        switch ($acfField['type']) {
            case 'image':
                $imageId = get_field($acfField['key'], $post->ID, false);
                $imageAlt = get_post_meta($imageId, '_wp_attachment_image_alt', true);
                $additionalData['image-alt'] = empty($imageAlt) ? '' : $imageAlt;
                $imageCaption = get_post_field('post_excerpt', $imageId);
                $additionalData['image-caption'] = empty($imageCaption) ? '' : $imageCaption;
                $imageTitle = get_the_title($imageId);
                $additionalData['image-title'] = empty($imageTitle) ? '' : $imageTitle;
