...
class LayoutPostTypeController extends Container implements Module
{
     ...
     * @var string
     */
    protected $dbMetaSlug = '_' . VCV_PREFIX . 'page-template';
    ...
    public function __construct()
    {
        ...
        $this->addFilter(
            ...
            'addWordpressDefaultsToPageTemplatesLayouts',
            10,
            1
        );
        $this->addFilter(
    ...
    /**
     * Add wordpress default layouts to our layout dropdow.
     *
     * @param array $themeTemplates
     *
     * @return array
     */
    protected function addWordpressDefaultsToPageTemplatesLayouts($themeTemplates)
    {
        for ($i = 0, $iMax = count($themeTemplates); $i < $iMax; $i++) {
            ...
            if ($themeTemplates[ $i ]['value'] === 'theme') {
                $themeTemplates[ $i ]['label'] = __('WordPress: Theme', 'visualcomposer');
                $themeTemplates[ $i ]['value'] = $themeTemplates[ $i ]['value'] . ':standard';
            }
        }
    ...
    {
        $layoutPostId = intval($request->input('vcv-source-id'));
        $response = ['status' => false];

        if (empty($layoutPostId)) {
            return $response;
        }

        $layoutPostList = $this->getLayoutsAttachedToPosts($layoutPostId);

        if (!empty($layoutPostList)) {
            $response['status'] = true;
            $response['vcvPostList'] = $layoutPostList;
        }

        $settingsList = $this->getSettingsAttachedGlobalToLayout($layoutPostId);
    ...
    /**
     * Get list of posts where layout is attached.
     *
     * @param int $layoutPostId
     *
     * @return array
     */
    protected function getLayoutsAttachedToPosts($layoutPostId)
    {
        global $wpdb;
        $postList = [];
        $query = $wpdb->prepare(
    ...
    /**
     * Get global settings title list where layout is attached.
     *
     * @param int $popupPostId
     *
     * @return array $settingsList
     */
    protected function getSettingsAttachedGlobalToLayout($layoutPostId)
    {
        $settingsList = [];
        ...
        foreach ($result as $settingsData) {
            ...
            $settingName = str_replace('vcv-custom-page-templates-', '', $settingName);
