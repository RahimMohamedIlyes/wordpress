...
class PostTypeController extends Container implements Module
{
    ...
    protected function trashPostLinkController($response, Request $request)
    {
        ...
        $query = $wpdb->prepare("SELECT post_title, ID FROM " . $wpdb->posts . " WHERE post_type != 'revision' AND post_content LIKE '%[vcv_global_template id=\"%d\"]%' LIMIT 10", $postId);
        ...
        if (!empty($result)) {
            ...
            foreach ($result as $postData) {
                $response['vcvPostList'][ $postData['ID']] = $postData['post_title'];
            }
        }
