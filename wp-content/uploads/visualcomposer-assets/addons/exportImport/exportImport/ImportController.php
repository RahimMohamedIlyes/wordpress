...
class ImportController extends Container implements Module
{
    ...
    /**
     * Media list id's and url that we need to add metadata for.
     *
     * @var array
     */
    protected $mediaListWithoutMetadata = [];
    ...
    public function __construct()
    {
        ...
        $this->addFilter('vcv:ajax:vcv:addon:exportImport:generateAttachmentMeta:adminNonce', 'generateAttachmentMeta');
        $this->addFilter('vcv:ajax:vcv:addon:exportImport:beginImport:adminNonce', 'beginImport');
     ...
     * @throws \VisualComposer\Framework\Illuminate\Container\BindingResolutionException
     */
    protected function beginImport(Request $requestHelper, Nonce $nonceHelper)
    {
    ...
    protected function import(
        ...
        if (!empty($this->items)) {
            ...
            if (empty($importProgress['statusMessages'])) {
                $statusMessages = [];
            } else {
                $statusMessages = $importProgress['statusMessages'];
            }

            return [
                'status' => true,
                'mediaListToProcess' => $this->mediaListWithoutMetadata,
                'statusMessages' => $statusMessages,
                'templateName' => $bundle['post']['post_title'],
                'templateLink' => get_edit_post_link($this->newTemplateId, '&'),
            ];
        }
    ...
    protected function addToMedia($newTemplateId, $localMediaPath, $fileHelper)
    {
        ...
        $this->mediaListWithoutMetadata[] = [
            'mediaId' => $attachment,
            'uploadPath' => $localMediaPath,
        ];
    ...
    protected function processDesignOptions($templateElements, $template, $newTemplateId)
    {
        ...
        foreach ($recursiveIterator as $key => $value) {
            ...
            if (!$this->isMediaBundleElement($key, $value) || $currentDepth <= 1) {
                continue;
            }
            ...
            $recursiveIterator = $this->updateDesignOptionsMediaUrlAndId($recursiveIterator, $newValue, $currentDepth);
        }
    ...
    /**
     * After create new media we need update urls and id for old ones.
     *
     ...
     * @return mixed
     */
    protected function updateDesignOptionsMediaUrlAndId($recursiveIterator, $newValue, $currentDepth)
    {
        $newMedia = [];
        ...
        for ($subDepth = $currentDepth; $subDepth >= 0; $subDepth--) {
            ...
            // use the replacements ($value) otherwise set the key to the parent iterators value
    ...
    /**
     * Check if current bundle element related to media type.
     *
     * @param string $key
     * @param mixed $value
     *
     * @return bool
     */
    protected function isMediaBundleElement($key, $value)
    {
        $keys = [
            'videoEmbed',
            'image',
        ...
        return is_array($value) && in_array($key, $keys, true) && isset($value['urls']);
    }
     ...
     * @throws \VisualComposer\Framework\Illuminate\Container\BindingResolutionException
     */
    protected function parseTemplateElements($bundle, $newTemplateId, $templateElements)
    {
     ...
     * @return string
     */
    protected function generateAttachmentMeta(Nonce $nonceHelper, Request $requestHelper)
    {
        if (!$nonceHelper->verifyAdmin($requestHelper->input('vcv-nonce'))) {
            return json_encode(
                [
        ...
        $mediaId = $requestHelper->input('vcv-media-id');
        $localMediaPath = $requestHelper->input('vcv-upload-path');
        $wpUploadDir = wp_upload_dir();
