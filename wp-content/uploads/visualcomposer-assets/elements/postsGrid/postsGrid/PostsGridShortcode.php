...
class PostsGridShortcode extends Container implements Module
{
    ...
    protected function render($atts, $content, $tag)
    {
        ...
        $excerptLength = $this->getExcerptLength($atts);

        $excerptLengthFilterCallback = $this->wpAddFilter('excerpt_length', function () use ($excerptLength) {
            return $excerptLength;
        }, 999);
        $excerptMoreFilterCallback = $this->wpAddFilter('excerpt_more', function () use ($excerptLength) {
            $excerptMore = '';
            if ($excerptLength) {
                $excerptMore = '&hellip;';
            }
        ...
        // we should get post output only after all filters that we override above
     ...
     * @return int
     */
    public function getExcerptLength($atts)
    {
        $defaultExcerptLength = (int)apply_filters('excerpt_length', '55');
        ...
        return (int)$atts['excerpt_length'];
    }
